...
export function Checkbox(props) {
    ...
    async function AddToPlaylist(type,event,data){
        if(type === "ADD-TO-PLAYLIST"){
            if(!CheckedState){
              await axios.post("https://KnightStream.ishanjirety.repl.co/api/playlist/item/add",{name:event.target.value,video:data})
              PlaylistDispatcher({type:"ADD-TO-PLAYLIST",payload:{data:data,name:event.target.value}})
              setCheckedState(true)
            }
            else if(CheckedState){
                await axios.post("https://KnightStream.ishanjirety.repl.co/api/playlist/item/remove",{name:event.target.value,video:data})
